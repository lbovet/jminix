...
/**
 * A cluster manager managing {{@link RmiServerConnectionProvider} instances. Default registry port
 * is 1099.
 *
 * @author bovetl
 * @version $Revision$
...
public class RmiClusterManager extends ClusterManager {
   ...
   *
   * @see
   *     org.jminix.server.cluster.ClusterManager#createNodeProvider(org.jminix.server.cluster.ClusterManager.Node)
   */
  @Override
  ...
  /**
   * The credential source to authenticate against other members (all must use the same
   * credentials). Also automatically set the ClusterManager secret to the provided password.
   */
