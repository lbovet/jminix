...
public class ConfigurableServerConnectionProvider extends AbstractMapServerConnectionProvider {
  ...
  /**
   * @param format a MessageFormat pattern to generate the key name from the server connection
   *     provider key {0} and the connection key {1}. Defaults to {0}-{1}.
   */
  public void setKeyFormat(String format) {
    keyFormat = format;
  }

  public synchronized void setServerConnectionProviders(
      Map<String, ServerConnectionProvider> providers) {
    providerMap = providers;
    providerKeys = new ArrayList<>(providers.keySet());
    Collections.sort(providerKeys);
  }

  public synchronized void addServerConnectionProvider(
      String key, ServerConnectionProvider provider) {
